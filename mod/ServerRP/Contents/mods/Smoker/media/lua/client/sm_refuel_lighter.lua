...
function TimedRefuelEmptyLighter(player, empty_lighter, item)
	ISTimedActionQueue.add(TimedRefuel:new(player, item, 200))
end

TimedRefuel = ISBaseTimedAction:derive("TimedRefuel")

function TimedRefuel:isValid()
	-- Оставляем без изменений
	return true;
end

function TimedRefuel:start()
	-- Что происходит в начале действия (может быть пустым)
end

function TimedRefuel:stop()
...
function TimedRefuel:new(player, item, time)
	local o = {}
	setmetatable(o, self)
	self.__index = self
...
local function checkInvItemEmptyLighter(player, context, worldobjects, item)
	...
	if lighter == "Lighter" and item:getUsedDelta() < 1 then
		check_lighter = true
	end
...
--- Показ контекстно меню на предмете...
	local option = context:addOption(getText("IGUI_Refuel_Lighter"), player, TimedRefuelEmptyLighter, lighter, item); --- надпись меню
...
local invContextMenuMenuRefuelEmptyLighter = function(_player, context, worldobjects, test)
	local playerObj = getSpecificPlayer(_player);

	for i,k in pairs(worldobjects) do
	-- inventory item list
		if instanceof(k, "InventoryItem") then
			checkInvItemEmptyLighter(playerObj, context, worldobjects, k);          
			elseif not instanceof(k, "InventoryItem") and k.items and #k.items > 1 then
			checkInvItemEmptyLighter(playerObj, context, worldobjects, k.items[1]);
		end
	end
end

Events.OnFillInventoryObjectContextMenu.Add(invContextMenuMenuRefuelEmptyLighter); -- контекстное меню для заправки зажигалки